...
hango({ nomCom: "hacker",
    ...
    if (!arg || arg == "") {
        repondre("*__Exemple : * " + prefixe + "hacker Eliahhango");
        return;
    }
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("* processing...*");
        await hn.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t Logo by ELIAH-AI*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
hango({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, hn, commandeOptions) => {
    ...
    try {
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await hn.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t Logo by Bmw-Md*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
hango({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, hn, commandeOptions) => {
    ...
    try {
        ...
        await hn.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by Bmw-md*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
hango({nomCom: "neonlight", categorie: "Logo", reaction: "💡"}, async (dest, hn, commandeOptions) => {
  ...
  try{
    ...
    /*.then((data) => {*/
     hn.sendMessage(dest, { image: { url: maker.image }, caption: 'Logo by Bmw-md' }, { quoted: ms });
...
hango({nomCom: "glitch", categorie: "Logo", reaction: "🎛️"}, async (dest, hn, commandeOptions) => {
  ...
  mumaker.textpro("https://textpro.me/create-impressive-glitch-text-effects-online-1027.html", text)
    .then((data) => {
      hn.sendMessage(dest, { image: { url: data.image }, caption: 'Logo by ELIAH-AI' }, { quoted: ms });
    })
    .catch(console.error);
});
...
repondre("  processing ...")
         await hn.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by ELIAH-AI*"})
         
        // await hn.sendMessage(dest,{image:{url:lienImage},caption:"\t *Logo by ELIAH-AI*"})
...
hango({nomCom:"gold",categorie:"Logo",reaction:"🧚🏿‍♀️"},async(dest,hn,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"gold ELIAH-AI");return;
      }
    ...
    var img = await mumaker.ephoto(lien,arg);
   repondre("processing ...")
    await hn.sendMessage(dest,{image:{url:img.image},caption:" *Logo by ELIAH-AI*"},{quoted:ms})
  }catch(e){repondre(e)}
})
...
hango({nomCom:"arena",categorie:"Logo",reaction:"🥵"},async(dest,hn,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"arena ELIAH-AI");return;
      }
