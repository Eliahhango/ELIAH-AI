  ...
  const clientId = 'b40a1820d63cd4e' ;
    ...
    try {
     ...
     if (msgRepondu.imageMessage) {
        const image = await hn.downloadAndSaveMediaMessage(msgRepondu.imageMessage)
         img = await uploadImageToImgur(image, clientId )
     } else {
        
        img = await hn.profilePictureUrl(auteurMsgRepondu, 'image'); }
      } else {
      ...
      await hn.sendMessage(origineMessage, { image: result }, { quoted: ms });
    } catch (error) {
