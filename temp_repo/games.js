...
hango({
  ...
  async (origineMessage, hn, commandeOptions) => {
    ...
    if (msgRepondu) {
        ...
        try {
            const repinv = await hn.awaitForMessage({
...
*Rules:* The game will start soon; you have a maximum of 1 minute each to make a choice in our private chat;`
      ...
      hn.sendMessage(origineMessage,{text : msg1,mentions : [auteurMessage, auteurMsgRepondu]} ) ;
...
let choix = [] ;
 ...
 try {
        ...
        for (const player of players) {
         ...
         hn.sendMessage(origineMessage,{ text : `@${player.split("@")[0]} Please go to this chat to make a choice
         https://wa.me/${idBot.split('@')[0]} ` , mentions : [player]})
            hn.sendMessage(player,{text : msg2}) ;
             
          const msgrecu =  await hn.awaitForMessage({
                sender: player,
                chatJid: player,
                timeout: 30000 // 30 secondes
            });
...
if (!choixPossibles.includes(choixJoueur1) || !choixPossibles.includes(choixJoueur2)) {
    ...
    hn.sendMessage(origineMessage,{ text : `*joueur 1 :* @${auteurMsgRepondu.split('@')[0]}
...
*resultat :* l'un ou les deux choix ne sont pas valides.`, mentions : [auteurMessage, auteurMsgRepondu] });
    ...
    // C'est une égalité
    hn.sendMessage(origineMessage,{ text : `*joueur 1 :* @${auteurMsgRepondu.split('@')[0]} a choisi(e) *${choixJoueur2}* 
...
resultat : il y'a donc match nul` , mentions : [auteurMessage, auteurMsgRepondu] });
    ...
    // Joueur 1 gagne
    hn.sendMessage(origineMessage,{ text : `*player 1 :* @${auteurMsgRepondu.split('@')[0]} choose *${choixJoueur2}* 
...
*result :* @${auteurMsgRepondu.split('@')[0]} win ` , mentions : [auteurMessage, auteurMsgRepondu] });
            ...
            if (error.message === 'Timeout') {
                ...
                hn.sendMessage(origineMessage,{ text : `*player 1 :* @${auteurMsgRepondu.split('@')[0]}
*player 2 :* @${auteurMessage.split('@')[0]}
...
` , mentions : [auteurMessage, auteurMsgRepondu]});
            ...
            if (error.message === 'Timeout') {
                // Le temps d'attente est écoulé
                hn.sendMessage(origineMessage,{ text : `@${auteurMsgRepondu.split('@')[0]} took too long to respond to the invitation from
...
Send the number off right answers`
       ...
       let rep = await  hn.awaitForMessage({
